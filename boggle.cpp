...
// returns stringstream that can be sent to cout in main.cpp
...
// uses the current time to seed the random number generator & then fills the array with random letters from alpha
...
bool Boggle::importBoard(string file_name)
{
    ...
    for(y = 0; y < SIZE; y++) {
        ...
        if(line.size() != SIZE*2-1) {
            cout << "line wrong size: actual " << ", expected: " << SIZE*2-1 << endl;
            return false;
        }
        for(x = 0; x < SIZE; x++) {
